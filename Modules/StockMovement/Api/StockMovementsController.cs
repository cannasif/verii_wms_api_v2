...
public sealed class StockMovementsController(IStockMovementService service, IPermissionAuthorizationService permissions) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Post(PostStockMovementRequest request, CancellationToken ct) { await Require("WMS.STOCK_MOVEMENTS.POST", ct); return Ok(ApiResponse<StockMovementPostResult>.Ok(await service.PostAsync(request, ct), "Stok hareketi kaydedildi.")); }
