    ...
    IPermissionAuthorizationService permissions):ControllerBase
{
    ...
    [HttpPost("decision-codes/paged")] public async Task<IActionResult> DecisionCodesPaged(PagedRequest request,CancellationToken ct){await Require("WMS.QUALITY.DECISION_CODES.VIEW",ct);return Ok(ApiResponse<PagedResponse<QualityDecisionCodeGridRow>>.Ok(await service.GetDecisionCodesPagedAsync(request,ct)));}
    [HttpPost("decision-codes/options/paged")] public async Task<IActionResult> DecisionCodeOptionsPaged([FromQuery]string branchCode,[FromQuery]QualityDecision decision,PagedRequest request,CancellationToken ct){await Require("WMS.QUALITY.INSPECTIONS.VIEW",ct);return Ok(ApiResponse<PagedResponse<QualityDecisionCodeOption>>.Ok(await service.GetDecisionCodeOptionsPagedAsync(branchCode,decision,request,ct)));}
    [HttpPost("decision-codes")] public async Task<IActionResult> CreateDecisionCode(QualityDecisionCodeUpsertRequest request,CancellationToken ct){await Require("WMS.QUALITY.DECISION_CODES.MANAGE",ct);return Ok(ApiResponse<object>.Ok(new{id=await service.CreateDecisionCodeAsync(request,UserId(),ct)},"Kalite karar kodu oluşturuldu."));}
    [HttpPut("decision-codes/{id:long}"),HttpPost("decision-codes/{id:long}/update")] public async Task<IActionResult> UpdateDecisionCode(long id,QualityDecisionCodeUpsertRequest request,CancellationToken ct){await Require("WMS.QUALITY.DECISION_CODES.MANAGE",ct);await service.UpdateDecisionCodeAsync(id,request,UserId(),ct);return Ok(ApiResponse<bool>.Ok(true,"Kalite karar kodu güncellendi."));}
    [HttpDelete("decision-codes/{id:long}"),HttpPost("decision-codes/{id:long}/delete")] public async Task<IActionResult> DeleteDecisionCode(long id,CancellationToken ct){await Require("WMS.QUALITY.DECISION_CODES.MANAGE",ct);await service.DeleteDecisionCodeAsync(id,UserId(),ct);return Ok(ApiResponse<bool>.Ok(true,"Kalite karar kodu silindi."));}
    ...
    [HttpPost("inspections/priority/reorder")] public async Task<IActionResult> ReorderPriority(ReorderQualityInspectionPriorityRequest request,CancellationToken ct){await Require("WMS.QUALITY.INSPECTIONS.PRIORITIZE",ct);var result=await service.ReorderInspectionPriorityAsync(request,UserId(),ct);return Ok(ApiResponse<QualityInspectionPriorityReorderResult>.Ok(result));}
