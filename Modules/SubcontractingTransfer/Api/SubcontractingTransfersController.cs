...
namespace verii_wms_api_v2.Modules.SubcontractingTransfer.Api;

[Authorize,ApiController,Route("api/subcontracting-transfers")]
public sealed class SubcontractingTransfersController(
    ISubcontractingTransferService service,IWarehouseTransferService transfers,
    IWarehouseTransferOperationService operations,IPermissionAuthorizationService permissions):ControllerBase
{
    ...
    [HttpPost("drafts")]
    public async Task<IActionResult>Create(CreateSubcontractingTransferDraftRequest request,CancellationToken ct){
        await Require("WMS.SUBCONTRACTING_TRANSFER.CREATE",ct);
        return Ok(ApiResponse<CreateWarehouseTransferDraftResult>.Ok(await service.CreateDraftAsync(request,UserId(),ct),"Fason transfer taslağı oluşturuldu."));}
    [HttpPost("paged")]
    ...
    public async Task<IActionResult>Update(long id,UpdateWarehouseTransferDraftRequest request,CancellationToken ct){
        await Require("WMS.SUBCONTRACTING_TRANSFER.UPDATE",ct);
        return Ok(ApiResponse<SubcontractingTransferDetail>.Ok(await service.UpdateDraftAsync(id,request,UserId(),ct),"Fason transfer taslağı güncellendi."));}
    [HttpDelete("{id:long}"),HttpPost("{id:long}/delete")]
    public async Task<IActionResult>Delete(long id,CancellationToken ct){
        await Require("WMS.SUBCONTRACTING_TRANSFER.DELETE",ct);await service.DeleteDraftAsync(id,UserId(),ct);
        return Ok(ApiResponse<bool>.Ok(true,"Fason transfer taslağı silindi."));}
    [HttpPost("{id:long}/approve")]
    public async Task<IActionResult>Approve(long id,WarehouseTransferTransitionRequest request,CancellationToken ct){
        await Require("WMS.SUBCONTRACTING_TRANSFER.APPROVE",ct);await Ensure(id,ct);
    ...
    public async Task<IActionResult>Operate(long id,string operation,WarehouseTransferOperationRequest request,CancellationToken ct){
        ...
        var result=operation switch{
            ...
            _=>throw AppException.BadRequest("Geçersiz fason transfer operasyonu.")};
    ...
    public async Task<IActionResult>Cancel(long id,WarehouseTransferTransitionRequest request,CancellationToken ct){
        await Require("WMS.SUBCONTRACTING_TRANSFER.CANCEL",ct);await Ensure(id,ct);
    ...
    public async Task<IActionResult>Policy([FromQuery]string branchCode,CancellationToken ct){
        await Require("WMS.SUBCONTRACTING_TRANSFER.SETTINGS.VIEW",ct);return Ok(ApiResponse<SubcontractingTransferPolicyDto>.Ok(await service.GetPolicyAsync(branchCode,ct)));}
    [HttpPut("policy")]
    public async Task<IActionResult>UpdatePolicy(UpdateSubcontractingTransferPolicyRequest request,CancellationToken ct){
        await Require("WMS.SUBCONTRACTING_TRANSFER.SETTINGS.MANAGE",ct);
        return Ok(ApiResponse<SubcontractingTransferPolicyDto>.Ok(await service.UpdatePolicyAsync(request,UserId(),ct),"Fason transfer politikası kaydedildi."));}
