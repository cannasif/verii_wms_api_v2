...
public sealed class ProcurementSupplierPortalService(IUnitOfWork uow):IProcurementSupplierPortalService
{
    ...
    {
        var invitation=await FindAsync(token,true,ct);EnsureUsable(invitation);
        ...
        await uow.SaveChangesAsync(ct);return Map(invitation);
    }
    ...
    private async Task SaveAsync(string token,SaveSupplierPortalQuoteRequest request,bool submit,CancellationToken ct)
    {
        try
        {
            ...
            {
                var invitation=await FindAsync(token,true,innerCt);EnsureUsable(invitation);
                ...
                Validate(invitation,request,submit);
    ...
    private static void EnsureUsable(ProcurementQuoteInvitation invitation)
    {
    ...
    private static void Validate(ProcurementQuoteInvitation invitation,SaveSupplierPortalQuoteRequest request,bool submit)
    {
        ...
        if(request.Lines.Select(x=>x.RfqLineId).Distinct().Count()!=request.Lines.Count||request.Lines.Any(x=>!rfqLines.TryGetValue(x.RfqLineId,out var line)||x.Quantity<=0||x.Quantity>line.RequestedQuantity||x.UnitPrice<0||x.DiscountRate is <0 or >100||x.VatRate<0))throw AppException.BadRequest("Teklif kalemleri geçersiz veya istenen miktarı aşıyor.");
    }
    ...
    private static SupplierPortalQuote Map(ProcurementQuoteInvitation invitation)
    {
        var quote=invitation.CurrentQuote;var values=quote?.Lines.ToDictionary(x=>x.ProcurementRfqLineId)??[];
        return new(invitation.Rfq.RfqNo,invitation.Rfq.Subject,invitation.Rfq.BuyerMessage,invitation.RfqSupplier.SupplierCodeSnapshot,invitation.RfqSupplier.SupplierNameSnapshot,invitation.Status.ToString(),invitation.Rfq.ResponseDueDate,invitation.ExpiresAtUtc,quote?.QuoteNo,quote?.QuoteDate,quote?.ValidUntil,quote?.CurrencyCode??"TRY",quote?.ExchangeRate??1,quote?.Note,quote?.RevisionNo??1,invitation.Rfq.Lines.OrderBy(x=>x.LineNo).Select(x=>{values.TryGetValue(x.Id,out var value);return new SupplierPortalLine(x.Id,x.LineNo,x.StockCodeSnapshot,x.StockNameSnapshot,x.UnitCode,x.RequestedQuantity,x.RequiredDate,value?.QuotedQuantity??x.RequestedQuantity,value?.UnitPrice??0,value?.DiscountRate??0,value?.VatRate??20,value?.DeliveryDate??x.RequiredDate);}).ToList());
    }
}
