...
public sealed class WarehouseInboundOperationsService(
    ...
    IAuditLogWriter audit) : IWarehouseInboundOperationsService
{
    private IGenericRepository<WarehouseInboundHeader> Headers => unitOfWork.Repository<WarehouseInboundHeader>();
    private IGenericRepository<WarehouseInboundExecution> Executions => unitOfWork.Repository<WarehouseInboundExecution>();

    public Task<ManualWarehouseInboundResult> CreateOrderlessTaskAsync(CreateManualWarehouseInboundRequest request, long actorUserId, CancellationToken cancellationToken = default) =>
        CreateAsync(request, actorUserId, direct: false, cancellationToken);

    public Task<ManualWarehouseInboundResult> CreateDirectReceiptAsync(CreateManualWarehouseInboundRequest request, long actorUserId, CancellationToken cancellationToken = default) =>
    ...
    public async Task<PagedResponse<WarehouseInboundGridRow>> GetPagedAsync(PagedRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var query = joined.Select(x => new WarehouseInboundGridRow(x.Header.Id, x.Header.BranchCode, x.Header.DocumentNo, x.Header.DocumentDate,
        ...
        return await query
            ...
            .ApplySort(request, nameof(WarehouseInboundGridRow.CreatedDate))
            .ToPagedResponseAsync(request, cancellationToken);
    }

    public async Task<WarehouseInboundDetail> GetDetailAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        var lineQuery = unitOfWork.Repository<WarehouseInboundLine>().Query().Where(x => x.GrHeaderId == id).OrderBy(x => x.LineNo);
        var detailLines = await lineQuery.Select(x => new WarehouseInboundDetailLine(x.Id, x.LineNo, x.StockId, x.StockCodeSnapshot,
            ...
            x.DefaultPutawayLocationId)).ToListAsync(cancellationToken);
        var sourceDocuments = await unitOfWork.Repository<WarehouseInboundSourceDocument>().Query().Where(x => x.GrHeaderId == id)
        ...
        var taskNumbers = await unitOfWork.Repository<WarehouseInboundTask>().Query().Where(x => x.GrHeaderId == id)
        ...
        var executionDimensions = await (from execution in Executions.Query()
            join executionLine in unitOfWork.Repository<WarehouseInboundExecutionLine>().Query()
                on execution.Id equals executionLine.GrExecutionId
            where execution.GrHeaderId == id && execution.Status == WarehouseInboundExecutionStatus.Posted
        ...
        var putawayCandidates = new List<WarehouseInboundPutawayCandidate>();
        foreach (var dimension in executionDimensions.OrderBy(x => x.Key.GrLineId).ThenBy(x => x.Key.SerialNo).ThenBy(x => x.Key.LotNo))
        {
            ...
            putawayCandidates.Add(new WarehouseInboundPutawayCandidate(
        ...
        var grid = new WarehouseInboundGridRow(header.Id, header.BranchCode, header.DocumentNo, header.DocumentDate,
        ...
        return new WarehouseInboundDetail(grid, detailLines, putawayCandidates, documents, taskNumbers, executionCount);
    }

    private Task<ManualWarehouseInboundResult> CreateAsync(CreateManualWarehouseInboundRequest request, long actor, bool direct, CancellationToken ct)
    {
        ...
        return unitOfWork.ExecuteInTransactionAsync(async token =>
        {
            ...
            var allocated = await numberAllocator.AllocateAsync(request.DocumentSeriesId, WmsDocumentType.WarehouseReceipt, DateTime.UtcNow, token);
            var now = DateTimeOffset.UtcNow;
            var header = Stamp(new WarehouseInboundHeader
            {
                BranchCode = branch, DocumentSeriesId = allocated.DocumentSeriesId, DocumentNo = allocated.DocumentNumber,
                DocumentDate = request.DocumentDate, ReceiptType = WarehouseInboundType.Direct,
                InitiationMode = direct ? WarehouseInboundInitiationMode.DirectReceipt : WarehouseInboundInitiationMode.UnplannedTask,
                ProcessType = direct ? WarehouseInboundProcessType.OrderlessDirectReceipt : WarehouseInboundProcessType.OrderlessTask,
            ...
            if (!string.IsNullOrWhiteSpace(header.WaybillNo))
                await unitOfWork.Repository<WarehouseInboundSourceDocument>().AddAsync(Stamp(new WarehouseInboundSourceDocument
                {
                    BranchCode = branch, Header = header, SourceDocumentType = WarehouseInboundSourceDocumentType.SupplierWaybill,
            ...
            if (!string.IsNullOrWhiteSpace(header.ElectronicWaybillNo) && !string.Equals(header.ElectronicWaybillNo, header.WaybillNo, StringComparison.OrdinalIgnoreCase))
                await unitOfWork.Repository<WarehouseInboundSourceDocument>().AddAsync(Stamp(new WarehouseInboundSourceDocument
                {
                    BranchCode = branch, Header = header, SourceDocumentType = WarehouseInboundSourceDocumentType.ElectronicWaybill,
            ...
            var grLines = new List<WarehouseInboundLine>();
            for (var index = 0; index < request.Lines.Count; index++)
            {
                ...
                var line = Stamp(new WarehouseInboundLine
                {
                    ...
                    Status = direct ? WarehouseInboundLineStatus.Received : WarehouseInboundLineStatus.Open,
            ...
            WarehouseInboundTask? task = null;
            if (!direct)
            {
                task = Stamp(new WarehouseInboundTask { BranchCode = branch, Header = header, TaskNo = TaskNo(header.DocumentNo),
                    TaskType = WarehouseInboundTaskType.Receive, Status = WarehouseInboundTaskStatus.Assigned, Priority = request.Priority,
                    WarehouseId = warehouse.Id, PlannedStartAtUtc = request.PlannedArrivalAtUtc?.ToUniversalTime() }, actor);
                header.Tasks.Add(task);
                for (var index = 0; index < grLines.Count; index++) task.Lines.Add(Stamp(new WarehouseInboundTaskLine
                {
                    ...
                    UnitCode = grLines[index].UnitCode, Status = WarehouseInboundTaskStatus.Assigned
                }, actor));
                ...
                foreach (var userId in users) task.Assignments.Add(Stamp(new WarehouseInboundTaskAssignment
                {
                    BranchCode = branch, Task = task, UserId = userId, AssignmentRole = userId == actor ? WarehouseInboundAssignmentRole.Owner : WarehouseInboundAssignmentRole.Worker,
                    Status = WarehouseInboundAssignmentStatus.Assigned, AssignedAtUtc = now, AssignedBy = actor
                }, actor));
            }

            header.StatusHistory.Add(Stamp(new WarehouseInboundStatusHistory { BranchCode = branch, Header = header,
                StatusArea = WarehouseInboundStatusArea.Operation, ToStatus = header.Status.ToString(), ChangedAtUtc = now,
            ...
            if (!direct)
            {
                await audit.WriteAsync(new("warehouse-inbound.create-orderless", nameof(WarehouseInboundHeader), header.Id.ToString(), "Succeeded", "warehouse-inbound",
    ...
    private async Task<ManualWarehouseInboundResult> PostDirectAsync(CreateManualWarehouseInboundRequest request, string requestHash, WarehouseInboundHeader header,
        IReadOnlyList<WarehouseInboundLine> lines, WarehouseEntity warehouse,
        IReadOnlyDictionary<long, ResolvedQualityPolicy> qualityPolicies, long actor, DateTimeOffset now, CancellationToken ct)
    {
        ...
        if (lines.Any(x => x.RequireQualityControl))
        {
            inspection = Stamp(new QualityInspection { BranchCode = header.BranchCode, CorrelationId = request.IdempotencyKey,
                InspectionNo = $"QC-{header.DocumentNo}", SourceDocumentType = "WarehouseInbound", SourceDocumentId = header.Id,
            ...
            foreach (var line in lines.Where(x => x.RequireQualityControl))
            {
                ...
                var qline = Stamp(new QualityInspectionLine { BranchCode = header.BranchCode, Inspection = inspection,
                    WarehouseInboundLineId = line.Id, StockId = line.StockId, StockCodeSnapshot = line.StockCodeSnapshot,
        ...
        var execution = Stamp(new WarehouseInboundExecution { BranchCode = header.BranchCode, Header = header,
            IdempotencyKey = request.IdempotencyKey, RequestHash = requestHash, ExecutionNo = $"{header.DocumentNo}-EX-01",
            Mode = request.ExecutionMode, Status = WarehouseInboundExecutionStatus.Posted,
        ...
        for (var index = 0; index < lines.Count; index++)
        {
            ...
            execution.Lines.Add(Stamp(new WarehouseInboundExecutionLine { BranchCode = header.BranchCode, Execution = execution,
                ...
                WarehouseInboundLabelId = input.WarehouseInboundLabelId,
        ...
        var movement = await stockMovementService.PostAsync(new PostStockMovementRequest(
            $"WI:{request.IdempotencyKey:N}", StockMovementTypes.Receipt, "WarehouseInbound", header.DocumentNo, header.Id,
            execution.OccurredAtUtc.UtcDateTime, "WarehouseInbound", request.Description,
        ...
        await unitOfWork.SaveChangesAsync(ct);
        await audit.WriteAsync(new("warehouse-inbound.direct", nameof(WarehouseInboundHeader), header.Id.ToString(), "Succeeded", "warehouse-inbound",
    ...
    private async Task<ManualWarehouseInboundResult> ExistingResult(WarehouseInboundHeader header, WarehouseInboundExecution? execution, CancellationToken ct)
    {
        var task = await unitOfWork.Repository<WarehouseInboundTask>().Query().FirstOrDefaultAsync(x => x.GrHeaderId == header.Id, ct);
        var lines = unitOfWork.Repository<WarehouseInboundLine>().Query().Where(x => x.GrHeaderId == header.Id);
        var inspection = await unitOfWork.Repository<QualityInspection>().Query().FirstOrDefaultAsync(x => x.SourceDocumentType == "WarehouseInbound" && x.SourceDocumentId == header.Id, ct);
        return new(header.Id, header.DocumentNo, header.InitiationMode, header.Status, task?.Id, task?.TaskNo,
            ...
            await lines.SumAsync(x => header.InitiationMode == WarehouseInboundInitiationMode.DirectReceipt ? x.ReceivedQuantity : x.ExpectedQuantity, ct), true);
    }

    private static void Validate(CreateManualWarehouseInboundRequest request, bool direct)
    {
    ...
    private static void ValidateTrackedLines(
        CreateManualWarehouseInboundRequest request,
