    ...
    IStringLocalizer<WarehouseInboundResource> localizer) : IWarehouseInboundService
{
    ...
    private async Task<List<(ReserveWarehouseInboundOrderLineRequest Request, WarehouseInboundOrderSourceLine Source)>> ApplyAutomaticSerialsAsync(
    ...
    {
        var result = new List<(ReserveWarehouseInboundOrderLineRequest, WarehouseInboundOrderSourceLine)>(items.Count);
        foreach (var item in items)
        {
            ...
            var generated = await serialNumberPolicyService.GenerateAsync(
                new(branch, stock.Id, quantity,
                    BuildSerialGenerationKey("WI", operationKey, item.Request.OrderNumber, item.Request.OrderId),
                    "WarehouseInboundOrderDraft", null),
                actor,
                ct);
            var planned = trackings.Count == 0
                ? generated.Serials.Select(x => new PlanWarehouseInboundTrackingRequest(
