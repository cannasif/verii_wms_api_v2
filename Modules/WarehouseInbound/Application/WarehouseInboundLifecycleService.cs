...
public sealed class WarehouseInboundLifecycleService(
    IUnitOfWork uow,
    IStockMovementService movements,
    IAuditLogWriter audit) : IWarehouseInboundLifecycleService
{
    private IGenericRepository<WarehouseInboundHeader> Headers => uow.Repository<WarehouseInboundHeader>();
    private IGenericRepository<WarehouseInboundStatusHistory> Histories => uow.Repository<WarehouseInboundStatusHistory>();
    private IGenericRepository<StockMovementOperation> MovementOperations => uow.Repository<StockMovementOperation>();

    public Task<WarehouseInboundLifecycleResult> ApproveAsync(
        long id,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return uow.ExecuteInTransactionAsync(async ct =>
        {
            ...
            Touch(header, actor);
            AddHistory(header, WarehouseInboundStatusArea.Approval, from, header.ApprovalStatus.ToString(),
    ...
    public Task<WarehouseInboundLifecycleResult> ShortCloseAsync(
        long id,
        ...
        return uow.ExecuteInTransactionAsync(async ct =>
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (line.ReceivedQuantity + line.ShortClosedQuantity >= line.ExpectedQuantity)
                    line.Status = WarehouseInboundLineStatus.ShortClosed;
            ...
            Touch(header, actor);
            AddHistory(header, WarehouseInboundStatusArea.Operation, null, "ShortClosed",
    ...
    public Task<WarehouseInboundLifecycleResult> PutawayAsync(
        long id,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var movementKey = $"WI:{id}:PUTAWAY:{request.IdempotencyKey:N}";
        ...
        return uow.ExecuteInTransactionAsync(async ct =>
        {
            ...
            RefreshCompletion(header, actor);
            Touch(header, actor);
            AddHistory(header, WarehouseInboundStatusArea.Putaway, null, header.PutawayStatus.ToString(),
                request.IdempotencyKey, hash, reason, actor);
            await SaveAsync(ct);
            var affected = normalizedLines.Sum(x => x.Quantity);
    ...
    public Task<WarehouseInboundLifecycleResult> CancelAsync(
        long id,
        WarehouseInboundTransitionRequest request,
        long actor,
        CancellationToken cancellationToken = default)
        ...
        return uow.ExecuteInTransactionAsync(async ct =>
        {
            ...
            if (await IsReplayAsync(header, request.IdempotencyKey, hash, ct))
            {
                var lastReplay = await MovementOperations.Query()
                    .Where(x => x.IdempotencyKey.StartsWith($"WI:{id}:CANCEL:{request.IdempotencyKey:N}:"))
            ...
            var qualityIds = await uow.Repository<QualityInspection>().Query()
                .Where(x => x.SourceDocumentType == "WarehouseInbound" && x.SourceDocumentId == id)
                .Select(x => x.Id).ToListAsync(ct);
            var operationIds = await MovementOperations.Query()
                .Where(x => x.OperationType != StockMovementTypes.Reversal
                    && ((x.ReferenceType == "WarehouseInbound" && x.ReferenceId == id)
            ...
            foreach (var operationId in operationIds)
            {
                var reversal = await movements.ReverseAsync(operationId,
                    new ReverseStockMovementRequest(
                        $"WI:{id}:CANCEL:{request.IdempotencyKey:N}:{operationId}",
                        reason,
                        DateTime.UtcNow), ct);
            ...
            foreach (var line in header.Lines) line.Status = WarehouseInboundLineStatus.Cancelled;
            foreach (var task in header.Tasks)
            {
                task.Status = WarehouseInboundTaskStatus.Cancelled;
                task.CancelledAtUtc = now;
                task.CancellationReason = reason;
                foreach (var taskLine in task.Lines) taskLine.Status = WarehouseInboundTaskStatus.Cancelled;
            }
            var executions = await uow.Repository<WarehouseInboundExecution>().Query(true)
                .Where(x => x.GrHeaderId == id && x.Status == WarehouseInboundExecutionStatus.Posted).ToListAsync(ct);
            foreach (var execution in executions) execution.Status = WarehouseInboundExecutionStatus.Reversed;

            var labels = await uow.Repository<WarehouseInboundLabel>().Query(true)
                .Where(x => x.GrHeaderId == id && x.Status != WarehouseInboundLabelStatus.Consumed
                    && x.Status != WarehouseInboundLabelStatus.Void).ToListAsync(ct);
            ...
            Touch(header, actor);
            AddHistory(header, WarehouseInboundStatusArea.Operation, from, header.Status.ToString(),
                request.IdempotencyKey, hash, reason, actor);
            await SaveAsync(ct);
    ...
    private async Task<WarehouseInboundHeader> LoadAsync(long id, CancellationToken ct) =>
    ...
    private static void AddHistory(
        WarehouseInboundHeader header,
        WarehouseInboundStatusArea area,
        string? from,
        string to,
        Guid key,
        string requestHash,
        string? reason,
        long actor) =>
        header.StatusHistory.Add(new WarehouseInboundStatusHistory
        {
    ...
    private static void RefreshCompletion(WarehouseInboundHeader header, long actor)
    {
    ...
    private static void ValidateTransition(long id, WarehouseInboundTransitionRequest request, bool requireReason)
    {
    ...
    private static void ApplyVersion(WarehouseInboundHeader header, string supplied)
    {
        byte[] expected;
    ...
    private static void Touch(WarehouseInboundHeader header, long actor)
    {
    ...
    private Task WriteAudit(
        ...
        audit.WriteAsync(new AuditLogWriteEntry(
            $"warehouse-inbound.{operation}",
            nameof(WarehouseInboundHeader),
            header.Id.ToString(),
            "Succeeded",
            "warehouse-inbound",
            NewValues: new
    ...
    private static WarehouseInboundLifecycleResult Result(
        WarehouseInboundHeader header,
        long? movementId,
        decimal quantity,
        bool replayed) =>
