...
public sealed class WarehouseInboundExecutionConfiguration : BaseEntityConfiguration<WarehouseInboundExecution>
{
    protected override void ConfigureEntity(EntityTypeBuilder<WarehouseInboundExecution> b)
    {
        b.ToTable("RII_WI_EXECUTION");
        ...
        b.HasOne<WarehouseInboundExecution>().WithMany().HasForeignKey(x => x.ReversalOfExecutionId).OnDelete(DeleteBehavior.Restrict);
        b.HasIndex(x => x.IdempotencyKey).IsUnique().HasDatabaseName("UX_RII_WI_EXECUTION_IDEMPOTENCY");
        b.HasIndex(x => new { x.BranchCode, x.ExecutionNo }).IsUnique().HasFilter("[IsDeleted] = 0").HasDatabaseName("UX_RII_WI_EXECUTION_BRANCH_NO");
        b.HasIndex(x => new { x.GrHeaderId, x.OccurredAtUtc }).HasDatabaseName("IX_RII_WI_EXECUTION_HEADER_TIME");
    }
}

public sealed class WarehouseInboundExecutionLineConfiguration : BaseEntityConfiguration<WarehouseInboundExecutionLine>
{
    protected override void ConfigureEntity(EntityTypeBuilder<WarehouseInboundExecutionLine> b)
    {
        b.ToTable("RII_WI_EXECUTION_LINE", table =>
        {
            table.HasCheckConstraint("CK_RII_WI_EXECUTION_LINE_NO", "[LineNo] > 0");
            table.HasCheckConstraint("CK_RII_WI_EXECUTION_LINE_QTY", "[Quantity] > 0");
        });
        ...
        b.HasOne<WarehouseInboundLabel>().WithMany().HasForeignKey(x => x.WarehouseInboundLabelId).OnDelete(DeleteBehavior.Restrict);
        ...
        b.HasIndex(x => new { x.GrExecutionId, x.LineNo }).IsUnique().HasFilter("[IsDeleted] = 0").HasDatabaseName("UX_RII_WI_EXECUTION_LINE_SEQUENCE");
        b.HasIndex(x => new { x.GrLineId, x.GrExecutionId }).HasDatabaseName("IX_RII_WI_EXECUTION_LINE_GR_LINE");
        b.HasIndex(x => new { x.StockId, x.LotNo, x.SerialNo }).HasDatabaseName("IX_RII_WI_EXECUTION_LINE_TRACE");
    }
}
