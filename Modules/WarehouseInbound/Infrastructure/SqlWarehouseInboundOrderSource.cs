...
public sealed class SqlWarehouseInboundOrderSource(WmsDbContext dbContext) : IWarehouseInboundOrderSource
{
    public async Task<IReadOnlyList<WarehouseInboundOrderSourceLine>> GetOpenLinesAsync(string orderNumbersCsv, string customerCode, string branchCode, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var result = new List<WarehouseInboundOrderSourceLine>();
            await using var reader = await command.ExecuteReaderAsync(cancellationToken);
            while (await reader.ReadAsync(cancellationToken))
            {
                result.Add(new WarehouseInboundOrderSourceLine(
