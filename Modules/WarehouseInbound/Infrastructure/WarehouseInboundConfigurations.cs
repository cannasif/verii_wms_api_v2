...
public sealed class WarehouseInboundHeaderConfiguration : BaseEntityConfiguration<WarehouseInboundHeader>
{
    protected override void ConfigureEntity(EntityTypeBuilder<WarehouseInboundHeader> builder)
    {
        builder.ToTable("RII_WI_HEADER", table =>
        {
            table.HasCheckConstraint("CK_RII_WI_HEADER_PRIORITY", "[Priority] BETWEEN 1 AND 5");
            table.HasCheckConstraint("CK_RII_WI_HEADER_OVER_TOLERANCE", "[OverReceiptTolerancePercent] BETWEEN 0 AND 100");
        });
        ...
        builder.Property(x => x.InitiationMode)
            .HasDefaultValue(WarehouseInboundInitiationMode.OrderBasedTask)
            .HasSentinel((WarehouseInboundInitiationMode)0);
        builder.Property(x => x.ProcessType)
            .HasDefaultValue(WarehouseInboundProcessType.OrderBasedTask)
            .HasSentinel((WarehouseInboundProcessType)0);
        builder.Property(x => x.LabelStrategy)
            .HasDefaultValue(WarehouseInboundLabelStrategy.None)
            .HasSentinel((WarehouseInboundLabelStrategy)0);
        ...
        builder.Property(x => x.ErpPostingPolicy).HasDefaultValue(WarehouseInboundErpPostingPolicy.AfterAllApprovals).HasSentinel((WarehouseInboundErpPostingPolicy)0);
...
public sealed class WarehouseInboundSourceDocumentConfiguration : BaseEntityConfiguration<WarehouseInboundSourceDocument>
{
    protected override void ConfigureEntity(EntityTypeBuilder<WarehouseInboundSourceDocument> builder)
    {
        builder.ToTable("RII_WI_SOURCE_DOCUMENT");
        ...
        builder.HasIndex(x => x.GrHeaderId).HasDatabaseName("IX_RII_WI_SOURCE_DOCUMENT_HEADER");
...
public sealed class WarehouseInboundLineConfiguration : BaseEntityConfiguration<WarehouseInboundLine>
{
    protected override void ConfigureEntity(EntityTypeBuilder<WarehouseInboundLine> builder)
    {
        builder.ToTable("RII_WI_LINE", table =>
        {
            table.HasCheckConstraint("CK_RII_WI_LINE_LINE_NO", "[LineNo] > 0");
            table.HasCheckConstraint("CK_RII_WI_LINE_UNIT_FACTOR", "[UnitConversionFactor] > 0");
            table.HasCheckConstraint("CK_RII_WI_LINE_QUANTITIES_NONNEGATIVE", "[ExpectedQuantity] >= 0 AND [ReceivedQuantity] >= 0 AND [AcceptedQuantity] >= 0 AND [RejectedQuantity] >= 0 AND [QuarantineQuantity] >= 0 AND [PutawayQuantity] >= 0 AND [ShortClosedQuantity] >= 0");
            table.HasCheckConstraint("CK_RII_WI_LINE_QUALITY_TOTAL", "[AcceptedQuantity] + [RejectedQuantity] + [QuarantineQuantity] <= [ReceivedQuantity]");
            table.HasCheckConstraint("CK_RII_WI_LINE_PUTAWAY_TOTAL", "[PutawayQuantity] <= [AcceptedQuantity]");
            table.HasCheckConstraint("CK_RII_WI_LINE_OVER_TOLERANCE", "[OverReceiptTolerancePercent] BETWEEN 0 AND 100");
        });
...
public sealed class WarehouseInboundLineSourceConfiguration : BaseEntityConfiguration<WarehouseInboundLineSource>
{
    protected override void ConfigureEntity(EntityTypeBuilder<WarehouseInboundLineSource> builder)
    {
        builder.ToTable("RII_WI_LINE_SOURCE", table =>
            table.HasCheckConstraint("CK_RII_WI_LINE_SOURCE_QUANTITIES", "[OrderedQuantity] >= 0 AND [PreviouslyReceivedQuantity] >= 0 AND [AllocatedQuantity] >= 0 AND [ReceivedQuantity] >= 0"));
        ...
        builder.HasIndex(x => x.GrSourceDocumentId).HasDatabaseName("IX_RII_WI_LINE_SOURCE_DOCUMENT");
    }

    private static void Quantity(PropertyBuilder<decimal> property) => property.HasPrecision(18, 6).IsRequired();
}

public sealed class WarehouseInboundStatusHistoryConfiguration : BaseEntityConfiguration<WarehouseInboundStatusHistory>
{
    protected override void ConfigureEntity(EntityTypeBuilder<WarehouseInboundStatusHistory> builder)
    {
        builder.ToTable("RII_WI_STATUS_HISTORY");
