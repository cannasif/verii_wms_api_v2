...
CREATE OR ALTER FUNCTION [dbo].[RII_FN_ITHALAT_ACIK_DOSYALAR]()
RETURNS TABLE
AS
RETURN
(
    SELECT
        ...
        X.CARI_KOD,
        Y.CARI_ISIM,
        X.CARI_KOD2 AS TESLIM_CARI_KOD,
        Z.CARI_ISIM AS TESLIM_CARI_ISIM
    FROM V3RIICO..TBLITHDOSYAMAS AS X WITH (NOLOCK)
    LEFT JOIN V3RIICO..TBLCASABIT AS Y WITH (NOLOCK)
        ON Y.CARI_KOD = X.CARI_KOD
    LEFT JOIN V3RIICO..TBLCASABIT AS Z WITH (NOLOCK)
        ON Z.CARI_KOD = X.CARI_KOD2
    WHERE X.KAPALI = 'H'
);
GO
