...
public interface IWarehouseOutboundReservationService
{
    Task ReserveAsync(WarehouseOutboundHeader header, string key, long actor, CancellationToken ct);
    Task ConsumeAsync(WarehouseOutboundHeader header, IReadOnlyDictionary<long, WarehouseOutboundOperationLineRequest> lines, string key, long actor, CancellationToken ct);
    Task ReleaseAllAsync(WarehouseOutboundHeader header, string key, string reason, long actor, CancellationToken ct);
}

public sealed class WarehouseOutboundReservationService(IStockBalanceService balances) : IWarehouseOutboundReservationService
{
    public async Task ReserveAsync(WarehouseOutboundHeader header, string key, long actor, CancellationToken ct)
    {
    ...
    public async Task ConsumeAsync(WarehouseOutboundHeader header, IReadOnlyDictionary<long, WarehouseOutboundOperationLineRequest> lines,
        string key, long actor, CancellationToken ct)
    {
        if (header.ReservationPolicy == WarehouseOutboundReservationPolicy.None) return;
        var drafts = new List<(WarehouseOutboundLine Line, WarehouseOutboundTracking? Tracking, StockReservationLineRequest Request)>();
        foreach (var line in header.Lines.Where(x => lines.ContainsKey(x.Id)))
        {
            ...
            WarehouseOutboundTracking? tracking = null;
    ...
    public async Task ReleaseAllAsync(WarehouseOutboundHeader header, string key, string reason, long actor, CancellationToken ct)
    {
    ...
    private static List<(WarehouseOutboundLine Line, WarehouseOutboundTracking? Tracking, StockReservationLineRequest Request)> Build(WarehouseOutboundHeader header, bool reserve)
    {
        var drafts = new List<(WarehouseOutboundLine, WarehouseOutboundTracking?, StockReservationLineRequest)>();
    ...
    private Task<StockReservationPostResult> Post(WarehouseOutboundHeader header, string key, string type, string reason,
        IReadOnlyCollection<(WarehouseOutboundLine Line, WarehouseOutboundTracking? Tracking, StockReservationLineRequest Request)> rows, CancellationToken ct) =>
        balances.PostReservationAsync(new(key, "WarehouseOutbound", header.Id, header.DocumentNo, type, reason, rows.Select(x => x.Request).ToList()), ct);

    private static void Apply(IEnumerable<(WarehouseOutboundLine Line, WarehouseOutboundTracking? Tracking, StockReservationLineRequest Request)> rows, long actor)
    {
        foreach (var row in rows)
        {
            row.Line.ReservedQuantity += row.Request.QuantityDelta;
            row.Line.Status = row.Line.ReservedQuantity > 0 ? WarehouseOutboundLineStatus.Reserved : row.Line.Status;
    ...
    private static StockReservationLineRequest Row(WarehouseOutboundHeader header, WarehouseOutboundLine line, long locationId, string? lot, string? serial, decimal delta) =>
