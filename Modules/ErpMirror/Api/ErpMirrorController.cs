...
public sealed class ErpMirrorController(IErpMirrorService service, IPermissionAuthorizationService permissions) : ControllerBase
{
    ...
    [HttpPost("yap-codes/paged")] public async Task<IActionResult> YapCodesPaged([FromBody] PagedRequest request, CancellationToken ct) { await Require("ERP.MIRROR.VIEW", ct); return Ok(ApiResponse<PagedResponse<YapCodeMirrorDto>>.Ok(await service.GetYapCodesPagedAsync(request, ct))); }
    ...
    [HttpPost("sync/yap-codes")] public async Task<IActionResult> SyncYapCodes(CancellationToken ct) { await Require("ERP.MIRROR.SYNC", ct); return Ok(ApiResponse<MirrorSyncResult>.Ok(await service.SyncYapCodesAsync(ct))); }
