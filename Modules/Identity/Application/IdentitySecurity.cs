...
public static class IdentitySecurity
{
    ...
    {
        if (password is null || password.Length < MinimumPasswordLength || password.Length > MaximumPasswordLength)
            throw AppException.BadRequest($"Şifre {MinimumPasswordLength}-{MaximumPasswordLength} karakter arasında olmalıdır.");
    }
}
